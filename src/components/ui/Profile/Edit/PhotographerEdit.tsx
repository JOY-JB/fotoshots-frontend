...
const PhotographerEditPage = ({ params }: IProps) => {
  const { id } = params;
  const { data, isLoading } = useGetPhotographerByIdQuery(id);
  const [updatePhotographer] = useUpdatePhotographerMutation();
  ...
  const onSubmit = async (data: any) => {
    try {
      const res = await updatePhotographer({ id, body: data }).unwrap();
